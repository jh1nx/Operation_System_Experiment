...
int ProgramManager::wait(int *retval)
{
    ...
    while (true)
    {
        ...
        while (item)
        {
            ...
            item = item->next;  // 提前获取下一个，因为当前item可能被释放
            ...
            if (child->parentPid == this->running->pid)
            {
                ...
                if (child->status == ProgramStatus::DEAD)
                {
                    ...
                    lastPid = child->pid;
                    releasePCB(child);
                    printf("recycle zombie process: %d (father: %d)\n",child->pid,child->parentPid);
                }
            }
        }

        // 如果没有子进程了，返回最后一个回收的PID或-1
        if (flag)
        {
